...
BEGIN
    ...
    DECLARE @CurrentToken uniqueidentifier;
    IF @InitialTranCount = 0 BEGIN TRANSACTION;
    ELSE SAVE TRANSACTION TBX_WorkQueue_Complete;
    BEGIN TRY
        SELECT @CurrentStatus=Status,@CurrentToken=ClaimToken
        ...
        SET Status='COMPLETED',CompletedAtUtc=SYSUTCDATETIME(),CompletedBy=ORIGINAL_LOGIN()
        WHERE WorkItemId=@WorkItemId AND Status='CLAIMED' AND ClaimToken=@ClaimToken;
