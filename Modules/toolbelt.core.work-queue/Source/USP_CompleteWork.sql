...
CREATE OR ALTER PROCEDURE [toolbelt_core].[USP_CompleteWork]
(
...
BEGIN
    ...
    BEGIN
        SELECT CAST('1.0' AS varchar(16)) AS HelpContractVersion,
               ...
               CAST(N'USP_CompleteWork' AS sysname) AS ObjectName,
               v.Section, v.Ordinal, v.ItemName, v.SqlDataType, v.IsRequired,
               v.IsNullable, v.DefaultValue, v.Description, v.ExampleSql
        FROM (VALUES
          (CAST('DESCRIPTION' AS varchar(32)),1,CAST(NULL AS sysname),CAST(NULL AS varchar(256)),CAST(NULL AS bit),CAST(NULL AS bit),CAST(NULL AS nvarchar(4000)),CAST(N'Schließt genau ein CLAIMED Work Item mit dem passenden ClaimToken als COMPLETED ab. Ein fachliches Arbeitsergebnis wird in E1a nicht gespeichert.' AS nvarchar(max)),CAST(NULL AS nvarchar(max))),
          ('PARAMETER',1,N'@WorkItemId','bigint',1,0,NULL,N'Eindeutige Queue-ID.',NULL),
          ('PARAMETER',2,N'@ClaimToken','uniqueidentifier',1,0,NULL,N'Vom erfolgreichen Claim zurückgegebenes Ownership-Token.',NULL),
          ...
          ('RESULT_COLUMN',3,N'Status','varchar(16)',0,0,NULL,N'Nach Erfolg stets COMPLETED.',NULL),
          ...
          ('RESULT_COLUMN',8,N'CompletedAtUtc','datetime2(7)',0,1,NULL,N'UTC-Zeitpunkt des Abschlusses.',NULL),
          ('RESULT_COLUMN',9,N'CompletedBy','sysname',0,1,NULL,N'Original-Login des Abschlusses.',NULL),
          ('RESULT_COLUMN',10,N'FailedAtUtc','datetime2(7)',0,1,NULL,N'Bei COMPLETED NULL.',NULL),
          ('RESULT_COLUMN',11,N'FailedBy','sysname',0,1,NULL,N'Bei COMPLETED NULL.',NULL),
          ('RESULT_COLUMN',12,N'FailureCode','varchar(64)',0,1,NULL,N'Bei COMPLETED NULL.',NULL),
          ('RESULT_COLUMN',13,N'FailureMessage','nvarchar(1000)',0,1,NULL,N'Bei COMPLETED NULL.',NULL),
          ('RESULT_COLUMN',14,N'RowVersion','binary(8)',0,0,NULL,N'Aktueller Concurrency-Marker.',NULL),
          ('RESULT_COLUMN',15,N'ClaimGeneration','bigint',0,0,NULL,N'Ownership-Generation des abgeschlossenen Claims.',NULL),
          ('RESULT_COLUMN',16,N'LeaseDurationSeconds','int',0,1,NULL,N'Lease-Dauer des abgeschlossenen Claims.',NULL),
          ('RESULT_COLUMN',17,N'LeaseUntilUtc','datetime2(7)',0,1,NULL,N'Lease-Grenze des abgeschlossenen Claims.',NULL),
          ('RESULT_COLUMN',18,N'LastHeartbeatAtUtc','datetime2(7)',0,1,NULL,N'Letzter Heartbeat des abgeschlossenen Claims.',NULL),
          ('RESULT_COLUMN',19,N'IsLeaseExpired','bit',0,0,NULL,N'Bei COMPLETED stets 0.',NULL),
          ...
          ('ERROR',1,N'51920-51924',NULL,NULL,NULL,NULL,N'Parameter-, Zustands-, Token- oder Transaktionsfehler.',NULL),
          ('EXAMPLE',1,NULL,NULL,NULL,NULL,NULL,N'Schließt einen zuvor beanspruchten synthetischen Auftrag ab.',N'EXEC toolbelt_core.USP_CompleteWork @WorkItemId=1, @ClaimToken=''00000000-0000-0000-0000-000000000001'';')
    ...
    IF @WorkItemId IS NULL OR @WorkItemId <= 0 OR @ClaimToken IS NULL
        THROW 51920, N'@WorkItemId und @ClaimToken sind erforderlich.', 1;
    IF XACT_STATE() = -1
        THROW 51924, N'Ein Work Item kann in einer uncommittable Caller-Transaktion nicht abgeschlossen werden.', 1;
    ...
    DECLARE @CurrentStatus varchar(16);
    DECLARE @CurrentToken uniqueidentifier,@LeaseUntilUtc datetime2(7),@NowUtc datetime2(7);
    IF @InitialTranCount = 0 BEGIN TRANSACTION;
    ELSE SAVE TRANSACTION TBX_WorkQueue_Complete;
    BEGIN TRY
        SELECT @CurrentStatus=Status,@CurrentToken=ClaimToken,@LeaseUntilUtc=LeaseUntilUtc
        FROM toolbelt_core.WorkItem WITH (UPDLOCK,HOLDLOCK)
        ...
        BEGIN
            IF OBJECT_ID(N'toolbelt_core.USP_PrepareResultTable',N'P') IS NULL THROW 51924,N'Für @ResultTable fehlt toolbelt.core.result-table.',2;
        ...
        SET Status='COMPLETED',CompletedAtUtc=@NowUtc,CompletedBy=ORIGINAL_LOGIN()
        WHERE WorkItemId=@WorkItemId AND Status='CLAIMED' AND ClaimToken=@ClaimToken AND LeaseUntilUtc>@NowUtc;
        IF @@ROWCOUNT<>1 THROW 51923,N'Der Claim-Zustand hat sich konkurrierend verändert.',2;
    ...
    BEGIN CATCH
        ...
        ELSE IF @InitialTranCount>0 AND XACT_STATE()=1 ROLLBACK TRANSACTION TBX_WorkQueue_Complete;
        THROW;
    END CATCH;
    IF @Debug>0 RAISERROR(N'USP_CompleteWork: Work Item abgeschlossen.',10,1) WITH NOWAIT;
