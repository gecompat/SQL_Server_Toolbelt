...
-- ============================================================================
-- Zweck:     Kontrollierte Deinstallation von toolbelt.core.generate-series
-- Modus:     SQLCMD
...
BEGIN
    THROW 51055, N'Die SQLCMD-Variable ConfirmNoExternalConsumers muss 0 oder 1 sein.', 1;
END;
...
BEGIN
    PRINT N'toolbelt.core.generate-series ist nicht als installiert registriert; keine Änderung erforderlich.';
    RETURN;
END;

IF @InstalledVersion COLLATE Latin1_General_100_BIN2 <> N'1.0.0'
BEGIN
    THROW 51053, N'Die installierte Modulversion ist diesem Uninstall-Skript nicht bekannt.', 1;
END;

IF @DeploymentMode NOT IN (N'local', N'central')
BEGIN
    THROW 51053, N'Der registrierte Deployment-Modus fehlt oder ist ungültig.', 1;
END;

IF @DeploymentMode = N'central' AND @ConfirmNoExternalConsumers <> 1
BEGIN
    THROW 51055, N'Bei zentraler Installation ist ConfirmNoExternalConsumers=1 als ausdrückliche Betreiberbestätigung erforderlich.', 1;
END;
...
WHERE dependencies.referenced_id IN
      (
          OBJECT_ID(N'toolbelt_core.TVF_GenerateSeriesBigInt')
        , OBJECT_ID(N'toolbelt_core.TVF_GenerateSeriesInt')
      )
...
BEGIN
    ...
    THROW 51056, @DependencyMessage, 1;
END;
...
BEGIN TRY
    ...
    BEGIN
        THROW 51057, N'Der installierte Modulstand hat sich seit dem Uninstall-Preflight verändert.', 1;
    END;
    ...
    /*
     * Der int-Wrapper hängt vom bigint-Kern ab und wird deshalb zuerst
     * entfernt.
     */
    VALUES (N'TVF_GenerateSeriesInt'), (N'TVF_GenerateSeriesBigInt');
    ...
    BEGIN
        ...
        BEGIN
            ...
            IF @DropSql IS NULL
            BEGIN
                THROW 51053, N'Ein Release-Objekt besitzt einen nicht unterstützten lokal veränderten Objekttyp.', 1;
            END;
