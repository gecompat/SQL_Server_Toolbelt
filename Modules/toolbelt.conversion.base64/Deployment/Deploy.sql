...
-- Modul:     toolbelt.conversion.base64 v1.1.0
...
IF OBJECT_ID(N'tempdb..#tbx_Base64ReleaseObjects', N'U') IS NOT NULL
BEGIN
    DROP TABLE #tbx_Base64ReleaseObjects;
END;

IF OBJECT_ID(N'tempdb..#tbx_Base64DeployState', N'U') IS NOT NULL
BEGIN
    DROP TABLE #tbx_Base64DeployState;
END;

CREATE TABLE #tbx_Base64ReleaseObjects
(
      ReleaseVersion nvarchar(64) NOT NULL
    , SchemaName     sysname      NOT NULL
    , ObjectName     sysname      NOT NULL
    , ObjectType     char(2)      NOT NULL
    , CONSTRAINT PK_tbx_Base64ReleaseObjects
          PRIMARY KEY (ReleaseVersion, SchemaName, ObjectName)
);

INSERT INTO #tbx_Base64ReleaseObjects
(
      ReleaseVersion
    , SchemaName
    , ObjectName
...
VALUES
      (N'1.0.0', N'toolbelt_conversion', N'SVF_Base64Encode', 'FN')
    , (N'1.0.0', N'toolbelt_conversion', N'SVF_Base64Decode', 'FN')
    , (N'1.1.0', N'toolbelt_conversion', N'TVF_Base64Encode', 'IF')
    , (N'1.1.0', N'toolbelt_conversion', N'TVF_Base64Decode', 'IF')
    , (N'1.1.0', N'toolbelt_conversion', N'SVF_Base64Encode', 'FN')
    , (N'1.1.0', N'toolbelt_conversion', N'SVF_Base64Decode', 'FN');

CREATE TABLE #tbx_Base64DeployState
(
...
BEGIN
    THROW 51040, N'Dieses Modul unterstützt ausschließlich SQL Server 2019, 2022 und 2025.', 1;
END;

IF @DeploymentMode NOT IN (N'local', N'central')
BEGIN
    THROW 51041, N'Die SQLCMD-Variable DeploymentMode muss local oder central sein.', 1;
END;
...
IF @InstalledVersion IS NOT NULL
   AND NOT EXISTS
       (
           SELECT 1
           FROM #tbx_Base64ReleaseObjects AS ro
           WHERE ro.ReleaseVersion COLLATE Latin1_General_100_BIN2
                     = @InstalledVersion COLLATE Latin1_General_100_BIN2
       )
BEGIN
    THROW 51043, N'Die installierte Modulversion ist diesem Deployment nicht als unterstütztes Vorgängerrelease bekannt.', 1;
END;
...
BEGIN
    ...
    BEGIN
        THROW 51042, N'Zum Anlegen von toolbelt_conversion fehlt CREATE SCHEMA in der Installationsdatenbank.', 1;
    END;
END;
ELSE IF HAS_PERMS_BY_NAME(N'toolbelt_conversion', N'SCHEMA', N'ALTER') <> 1
BEGIN
    THROW 51042, N'Für das vorhandene Schema toolbelt_conversion fehlt ALTER.', 1;
END;

IF HAS_PERMS_BY_NAME(DB_NAME(), N'DATABASE', N'CREATE FUNCTION') <> 1
BEGIN
    THROW 51042, N'In der Installationsdatenbank fehlt CREATE FUNCTION.', 1;
END;
...
FROM #tbx_Base64ReleaseObjects AS target
...
WHERE target.ReleaseVersion = @TargetVersion
  ...
  (
      ...
      OR NOT EXISTS
         (
             SELECT 1
             FROM #tbx_Base64ReleaseObjects AS previous
...
BEGIN
    ...
    THROW 51044, @CollisionMessage, 1;
END;

INSERT INTO #tbx_Base64DeployState
(
      TargetVersion
    , InstalledVersion
    , DeploymentMode
...
BEGIN TRY
    ...
    EXEC @LockResult = sys.sp_getapplock
          @Resource    = N'toolbelt.deploy.toolbelt.conversion.base64'
    ...
    IF @LockResult < 0
    BEGIN
        THROW 51047, N'Ein paralleles Deployment von toolbelt.conversion.base64 ist bereits aktiv.', 1;
    END;
    ...
    BEGIN
        THROW 51047, N'Der installierte Modulstand hat sich seit dem Preflight verändert.', 1;
    END;
    ...
    IF @CollisionObject IS NOT NULL
    BEGIN
        THROW 51047, N'Ein Zielobjekt hat sich seit dem Preflight verändert oder ist neu hinzugekommen.', 1;
    END;
        ...
        (
            (
                SELECT
                    ...
                    + N';'
                FROM #tbx_Base64ReleaseObjects AS target
                INNER JOIN #tbx_Base64ReleaseObjects AS previous
...
:r ../Source/TVF_Base64Encode.sql
:r ../Source/TVF_Base64Decode.sql
:r ../Source/SVF_Base64Encode.sql
:r ../Source/SVF_Base64Decode.sql
...
BEGIN TRY
    ...
    FROM #tbx_Base64DeployState;

    IF XACT_STATE() <> 1
       OR OBJECT_ID(N'toolbelt_conversion.TVF_Base64Encode', N'IF') IS NULL
    ...
    BEGIN
        THROW 51048, N'Die Framework-Funktionen wurden nicht vollständig innerhalb der Deployment-Transaktion angelegt.', 1;
    END;
