...
VALUES
      (N'TVF_TruncateDateTimeOffset', 'IF')
    , (N'TVF_TruncateDateTime2', 'IF')
    , (N'TVF_TruncateDate', 'IF');

IF @ProductMajorVersion NOT IN (15, 16, 17)
    THROW 51230, N'Dieses Modul unterstützt ausschließlich SQL Server 2019, 2022 und 2025.', 1;
IF @DeploymentMode NOT IN (N'local', N'central')
    THROW 51231, N'Die SQLCMD-Variable DeploymentMode muss local oder central sein.', 1;
...
IF @InstalledVersion IS NOT NULL AND @InstalledVersion <> N'1.0.0'
    THROW 51233, N'Die installierte Modulversion ist diesem Deployment nicht bekannt.', 1;
...
IF @InstalledVersion IS NULL
   AND EXISTS
    ...
    THROW 51234, N'Ein Zielobjekt stammt nicht aus einem bekannten Toolbelt-Release.', 1;

IF SCHEMA_ID(N'toolbelt_datetime') IS NULL
   AND HAS_PERMS_BY_NAME(DB_NAME(), N'DATABASE', N'CREATE SCHEMA') <> 1
    THROW 51232, N'In der Installationsdatenbank fehlt CREATE SCHEMA.', 1;
IF SCHEMA_ID(N'toolbelt_datetime') IS NOT NULL
   AND HAS_PERMS_BY_NAME(N'toolbelt_datetime', N'SCHEMA', N'ALTER') <> 1
    THROW 51232, N'Für toolbelt_datetime fehlt ALTER.', 1;
IF HAS_PERMS_BY_NAME(DB_NAME(), N'DATABASE', N'CREATE FUNCTION') <> 1
    THROW 51232, N'In der Installationsdatenbank fehlt CREATE FUNCTION.', 1;

BEGIN TRANSACTION;
DECLARE @LockResult int;
EXEC @LockResult = sys.sp_getapplock
      @Resource = N'toolbelt.deploy.toolbelt.datetime.truncate'
    , @LockMode = N'Exclusive'
    , @LockOwner = N'Transaction'
    , @LockTimeout = 0
    , @DbPrincipal = N'public';
IF @LockResult < 0
    THROW 51237, N'Ein paralleles Deployment dieses Moduls ist bereits aktiv.', 1;
...
END;
GO
:r ../Source/TVF_TruncateDateTimeOffset.sql
:r ../Source/TVF_TruncateDateTime2.sql
:r ../Source/TVF_TruncateDate.sql
SET NOCOUNT ON;
BEGIN TRY
    DECLARE
          @VersionProperty sysname =
