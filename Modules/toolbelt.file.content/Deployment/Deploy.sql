...
IF SCHEMA_ID(N'toolbelt_file') IS NOT NULL
   AND HAS_PERMS_BY_NAME(N'toolbelt_file', N'SCHEMA', N'ALTER') <> 1
    THROW 51272, N'Für toolbelt_file fehlt ALTER.', 1;
...
IF HAS_PERMS_BY_NAME(DB_NAME(), N'DATABASE', N'CREATE TABLE') <> 1
    THROW 51272, N'In der Installationsdatenbank fehlt CREATE TABLE.', 1;
...
IF SCHEMA_ID(N'toolbelt_file') IS NULL
BEGIN
    EXEC sys.sp_executesql N'CREATE SCHEMA [toolbelt_file];';
    EXEC sys.sp_addextendedproperty
          @name = N'Toolbelt.Managed', @value = 1
        , @level0type = N'SCHEMA', @level0name = N'toolbelt_file';
    EXEC sys.sp_addextendedproperty
          @name = N'Toolbelt.SchemaCategory', @value = N'file'
        , @level0type = N'SCHEMA', @level0name = N'toolbelt_file';
END;
GO
:r ../Source/FileContentRootAllowlist.sql
:r ../Source/USP_LoadBinaryFile.sql
:r ../Source/USP_LoadTextFile.sql
SET NOCOUNT ON;
BEGIN TRY
    DECLARE
          @VersionProperty sysname =
