...
-- ============================================================================
-- Zweck:     Kontrollierte Deinstallation von toolbelt.conversion.integer-base
-- Modus:     SQLCMD
...
BEGIN
    THROW 51095, N'Die SQLCMD-Variable ConfirmNoExternalConsumers muss 0 oder 1 sein.', 1;
END;
...
BEGIN
    PRINT N'toolbelt.conversion.integer-base ist nicht als installiert registriert; keine Änderung erforderlich.';
    RETURN;
END;
...
BEGIN
    THROW 51093, N'Die installierte Modulversion ist diesem Uninstall-Skript nicht bekannt.', 1;
END;

IF @DeploymentMode NOT IN (N'local', N'central')
BEGIN
    THROW 51093, N'Der registrierte Deployment-Modus fehlt oder ist ungültig.', 1;
END;

IF @DeploymentMode = N'central' AND @ConfirmNoExternalConsumers <> 1
BEGIN
    THROW 51095, N'Bei zentraler Installation ist ConfirmNoExternalConsumers=1 als ausdrückliche Betreiberbestätigung erforderlich.', 1;
END;
...
WHERE dependencies.referenced_id IN
      (
          OBJECT_ID(N'toolbelt_conversion.TVF_IntegerToBase')
        , OBJECT_ID(N'toolbelt_conversion.TVF_TryBaseToInteger')
        , OBJECT_ID(N'toolbelt_conversion.SVF_IntegerToBase')
        , OBJECT_ID(N'toolbelt_conversion.SVF_TryBaseToInteger')
      )
...
BEGIN
    ...
    THROW 51096, @DependencyMessage, 1;
END;
...
BEGIN TRY
    ...
    BEGIN
        THROW 51097, N'Der installierte Modulstand hat sich seit dem Uninstall-Preflight verändert.', 1;
    END;
    ...
    BEGIN
        ...
        BEGIN
            ...
            IF @DropSql IS NULL
            BEGIN
                THROW 51093, N'Ein Release-Objekt besitzt einen nicht unterstützten lokal veränderten Objekttyp.', 1;
            END;
    ...
    BEGIN
        ...
        IF ISNULL(@SchemaManaged, 0) = 1
           AND ISNULL(@SchemaCategory, N'')
                   COLLATE Latin1_General_100_BIN2 = N'conversion'
           AND NOT EXISTS
               (
        ...
        BEGIN
            DROP SCHEMA [toolbelt_conversion];
        END;
    END;
