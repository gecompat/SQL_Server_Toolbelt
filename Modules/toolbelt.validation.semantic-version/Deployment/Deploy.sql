...
SET NOCOUNT ON;
SET XACT_ABORT ON;
SET QUOTED_SEMANTIC_VERSION ON;

IF OBJECT_ID(N'tempdb..#tbx_SemanticVersionReleaseObjects', N'U') IS NOT NULL
BEGIN
    DROP TABLE #tbx_SemanticVersionReleaseObjects;
END;

IF OBJECT_ID(N'tempdb..#tbx_SemanticVersionDeployState', N'U') IS NOT NULL
BEGIN
    DROP TABLE #tbx_SemanticVersionDeployState;
END;

CREATE TABLE #tbx_SemanticVersionReleaseObjects
(
      ReleaseVersion nvarchar(64) NOT NULL
    , SchemaName     sysname      NOT NULL
    , ObjectName     sysname      NOT NULL
    , ObjectType     char(2)      NOT NULL
    , CONSTRAINT PK_tbx_SemanticVersionReleaseObjects
          PRIMARY KEY (ReleaseVersion, SchemaName, ObjectName)
);

INSERT INTO #tbx_SemanticVersionReleaseObjects
(
      ReleaseVersion
    , SchemaName
    , ObjectName
...
DECLARE
    ...
    , @VersionPropertyName  sysname =
          N'Toolbelt.Module.toolbelt.validation.semantic-version.Version'
    , @ProductMajorVersion  int =
          TRY_CONVERT(int, SERVERPROPERTY(N'ProductMajorVersion'))
    , @SchemaId             int = SCHEMA_ID(N'toolbelt_validation')
...
BEGIN
    THROW 51080, N'Dieses Modul unterstützt ausschließlich SQL Server 2019, 2022 und 2025.', 1;
END;

IF @DeploymentMode NOT IN (N'local', N'central')
BEGIN
    THROW 51081, N'Die SQLCMD-Variable DeploymentMode muss local oder central sein.', 1;
END;
...
IF @InstalledVersion IS NOT NULL
   AND NOT EXISTS
       (
           SELECT 1
           FROM #tbx_SemanticVersionReleaseObjects AS ro
           WHERE ro.ReleaseVersion COLLATE Latin1_General_100_BIN2
                     = @InstalledVersion COLLATE Latin1_General_100_BIN2
       )
BEGIN
    THROW 51083, N'Die installierte Modulversion ist diesem Deployment nicht als unterstütztes Vorgängerrelease bekannt.', 1;
END;
...
BEGIN
    ...
    BEGIN
        THROW 51082, N'Zum Anlegen von toolbelt_validation fehlt CREATE SCHEMA in der Installationsdatenbank.', 1;
    END;
END;
ELSE IF HAS_PERMS_BY_NAME(N'toolbelt_validation', N'SCHEMA', N'ALTER') <> 1
BEGIN
    THROW 51082, N'Für das vorhandene Schema toolbelt_validation fehlt ALTER.', 1;
END;

IF HAS_PERMS_BY_NAME(DB_NAME(), N'DATABASE', N'CREATE FUNCTION') <> 1
BEGIN
    THROW 51082, N'In der Installationsdatenbank fehlt CREATE FUNCTION.', 1;
END;
...
FROM #tbx_SemanticVersionReleaseObjects AS target
...
WHERE target.ReleaseVersion = @TargetVersion
  ...
  (
      ...
      OR NOT EXISTS
         (
             SELECT 1
             FROM #tbx_SemanticVersionReleaseObjects AS previous
...
BEGIN
    ...
    THROW 51084, @CollisionMessage, 1;
END;

INSERT INTO #tbx_SemanticVersionDeployState
(
      TargetVersion
    , InstalledVersion
    , DeploymentMode
...
BEGIN TRY
    ...
    EXEC @LockResult = sys.sp_getapplock
          @Resource    = N'toolbelt.deploy.toolbelt.validation.semantic-version'
    ...
    IF @LockResult < 0
    BEGIN
        THROW 51087, N'Ein paralleles Deployment von toolbelt.validation.semantic-version ist bereits aktiv.', 1;
    END;
    ...
    BEGIN
        THROW 51087, N'Der installierte Modulstand hat sich seit dem Preflight verändert.', 1;
    END;
    ...
    IF @CollisionObject IS NOT NULL
    BEGIN
        THROW 51087, N'Ein Zielobjekt hat sich seit dem Preflight verändert oder ist neu hinzugekommen.', 1;
    END;

    IF SCHEMA_ID(N'toolbelt_validation') IS NULL
    BEGIN
        EXEC sys.sp_executesql N'CREATE SCHEMA [toolbelt_validation];';
        SET @SchemaCreated = 1;
        ...
        EXEC sys.sp_addextendedproperty
            ...
            , @value      = 1
            , @level0type = N'SCHEMA'
            , @level0name = N'toolbelt_validation';
    ...
    UPDATE #tbx_SemanticVersionDeployState
        ...
        (
            (
                SELECT
                    ...
                    + N';'
                FROM #tbx_SemanticVersionReleaseObjects AS target
                INNER JOIN #tbx_SemanticVersionReleaseObjects AS previous
...
BEGIN TRY
    DECLARE
        ...
        , @ModePropertyName    sysname =
              N'Toolbelt.Module.toolbelt.validation.semantic-version.DeploymentMode';

    SELECT
          @DeploymentMode = DeploymentMode
        , @TargetVersion = TargetVersion
    FROM #tbx_SemanticVersionDeployState;
    ...
    BEGIN
        THROW 51088, N'Die Framework-Funktionen wurden nicht vollständig innerhalb der Deployment-Transaktion angelegt.', 1;
    END;
    ...
    BEGIN
        ...
        (
            QUOTENAME(N'toolbelt_validation') + N'.' + QUOTENAME(@ObjectName)
        );
        ...
        VALUES
              (1, N'Toolbelt.ModuleId', N'toolbelt.validation.semantic-version')
