...
-- ============================================================================
-- Zweck:     Kontrollierte Deinstallation von toolbelt.validation.semantic-version
-- Modus:     SQLCMD
...
BEGIN
    THROW 51085, N'Die SQLCMD-Variable ConfirmNoExternalConsumers muss 0 oder 1 sein.', 1;
END;
...
BEGIN
    PRINT N'toolbelt.validation.semantic-version ist nicht als installiert registriert; keine Änderung erforderlich.';
    RETURN;
END;
...
BEGIN
    THROW 51083, N'Der registrierte Deployment-Modus fehlt oder ist ungültig.', 1;
END;

IF @DeploymentMode = N'central' AND @ConfirmNoExternalConsumers <> 1
BEGIN
    THROW 51085, N'Bei zentraler Installation ist ConfirmNoExternalConsumers=1 als ausdrückliche Betreiberbestätigung erforderlich.', 1;
END;
...
BEGIN
    ...
    THROW 51086, @DependencyMessage, 1;
END;
...
BEGIN TRY
    ...
    BEGIN
        THROW 51087, N'Der installierte Modulstand hat sich seit dem Uninstall-Preflight verändert.', 1;
    END;
        ...
        BEGIN
            ...
            SET @DropSql =
                CASE
                    ...
                    ELSE NULL
                END
                + QUOTENAME(N'toolbelt_validation')
                + N'.'
            ...
            IF @DropSql IS NULL
            BEGIN
                THROW 51083, N'Ein Release-Objekt besitzt einen nicht unterstützten lokal veränderten Objekttyp.', 1;
            END;
    ...
    BEGIN
        ...
        IF ISNULL(@SchemaManaged, 0) = 1
           AND ISNULL(@SchemaCategory, N'')
                   COLLATE Latin1_General_100_BIN2 = N'validation'
           AND NOT EXISTS
               (
        ...
        BEGIN
            DROP SCHEMA [toolbelt_validation];
        END;
    END;
