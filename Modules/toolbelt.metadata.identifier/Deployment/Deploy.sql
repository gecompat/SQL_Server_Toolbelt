...
-- Modul:     toolbelt.metadata.identifier v1.0.0
...
IF OBJECT_ID(N'tempdb..#tbx_IdentifierReleaseObjects', N'U') IS NOT NULL
BEGIN
    DROP TABLE #tbx_IdentifierReleaseObjects;
END;

IF OBJECT_ID(N'tempdb..#tbx_IdentifierDeployState', N'U') IS NOT NULL
BEGIN
    DROP TABLE #tbx_IdentifierDeployState;
END;

CREATE TABLE #tbx_IdentifierReleaseObjects
(
      ReleaseVersion nvarchar(64) NOT NULL
    , SchemaName     sysname      NOT NULL
    , ObjectName     sysname      NOT NULL
    , ObjectType     char(2)      NOT NULL
    , CONSTRAINT PK_tbx_IdentifierReleaseObjects
          PRIMARY KEY (ReleaseVersion, SchemaName, ObjectName)
);

INSERT INTO #tbx_IdentifierReleaseObjects
(
      ReleaseVersion
    , SchemaName
    , ObjectName
...
VALUES
      (N'1.0.0', N'toolbelt_metadata', N'TVF_ParseMultipartName', 'TF')
    , (N'1.0.0', N'toolbelt_metadata', N'SVF_QuoteMultipartName', 'FN');

CREATE TABLE #tbx_IdentifierDeployState
(
...
BEGIN
    THROW 51060, N'Dieses Modul unterstützt ausschließlich SQL Server 2019, 2022 und 2025.', 1;
END;

IF @DeploymentMode NOT IN (N'local', N'central')
BEGIN
    THROW 51061, N'Die SQLCMD-Variable DeploymentMode muss local oder central sein.', 1;
END;
...
IF @InstalledVersion IS NOT NULL
   AND NOT EXISTS
       (
           SELECT 1
           FROM #tbx_IdentifierReleaseObjects AS ro
           WHERE ro.ReleaseVersion COLLATE Latin1_General_100_BIN2
                     = @InstalledVersion COLLATE Latin1_General_100_BIN2
       )
BEGIN
    THROW 51063, N'Die installierte Modulversion ist diesem Deployment nicht als unterstütztes Vorgängerrelease bekannt.', 1;
END;
...
BEGIN
    ...
    BEGIN
        THROW 51062, N'Zum Anlegen von toolbelt_metadata fehlt CREATE SCHEMA in der Installationsdatenbank.', 1;
    END;
END;
ELSE IF HAS_PERMS_BY_NAME(N'toolbelt_metadata', N'SCHEMA', N'ALTER') <> 1
BEGIN
    THROW 51062, N'Für das vorhandene Schema toolbelt_metadata fehlt ALTER.', 1;
END;

IF HAS_PERMS_BY_NAME(DB_NAME(), N'DATABASE', N'CREATE FUNCTION') <> 1
BEGIN
    THROW 51062, N'In der Installationsdatenbank fehlt CREATE FUNCTION.', 1;
END;
...
FROM #tbx_IdentifierReleaseObjects AS target
...
WHERE target.ReleaseVersion = @TargetVersion
  ...
  (
      ...
      OR NOT EXISTS
         (
             SELECT 1
             FROM #tbx_IdentifierReleaseObjects AS previous
...
BEGIN
    ...
    THROW 51064, @CollisionMessage, 1;
END;

INSERT INTO #tbx_IdentifierDeployState
(
      TargetVersion
    , InstalledVersion
    , DeploymentMode
...
BEGIN TRY
    ...
    BEGIN
        THROW 51067, N'Der installierte Modulstand hat sich seit dem Preflight verändert.', 1;
    END;
    ...
    IF @CollisionObject IS NOT NULL
    BEGIN
        THROW 51067, N'Ein Zielobjekt hat sich seit dem Preflight verändert oder ist neu hinzugekommen.', 1;
    END;
    ...
    UPDATE #tbx_IdentifierDeployState
        ...
        (
            (
                SELECT
                    ...
                    + N';'
                FROM #tbx_IdentifierReleaseObjects AS target
                INNER JOIN #tbx_IdentifierReleaseObjects AS previous
...
:r ../Source/TVF_ParseMultipartName.sql
:r ../Source/SVF_QuoteMultipartName.sql
...
BEGIN TRY
    ...
    FROM #tbx_IdentifierDeployState;
    ...
    BEGIN
        THROW 51068, N'Die Framework-Funktionen wurden nicht vollständig innerhalb der Deployment-Transaktion angelegt.', 1;
    END;
    ...
    VALUES (N'TVF_ParseMultipartName'), (N'SVF_QuoteMultipartName');
    ...
    BEGIN
        ...
        VALUES
              (1, N'Toolbelt.ModuleId', N'toolbelt.metadata.identifier')
