-- ============================================================================
-- Objekt: toolbelt_string.TVF_TrimDirectionalVarchar
-- Zweck:  LEADING-, TRAILING- und BOTH-Trim für varchar-Text.
-- ============================================================================
CREATE OR ALTER FUNCTION [toolbelt_string].[TVF_TrimDirectionalVarchar]
(
      @Value      varchar(max)
...
(
    ...
    (
        SELECT
              InputLength = CONVERT(bigint, DATALENGTH(@Value))
    ...
    (
        ...
        WHERE CASE
            WHEN ASCII(SUBSTRING(@Value COLLATE Latin1_General_100_BIN2, positions.Value, 1)) = 0 THEN 0
    ...
    SELECT Value = CONVERT(varchar(max), CASE
        WHEN @Value IS NULL OR @Characters IS NULL THEN NULL
        WHEN parameters.IsValid = 0 THEN CONVERT(varchar(max), 1 / 0)
        WHEN parameters.InputLength = 0 THEN ''
        WHEN bounds.FirstValue IS NULL THEN ''
