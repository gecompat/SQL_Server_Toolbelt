...
**Status:** Research abgeschlossen und auf den implementierten ResultTable-/Dokumentationsstand konsolidiert
...
- Aus dem Vergleich entstehen zunächst ein abfragbarer Capability-/Versionskatalog (`TC-2026-023`) und URI-Percent-Encoding/-Decoding (`TC-2026-024`). Der bereits vorhandene Base64-Kandidat `TC-2026-012` wird durch die aktuelle native SQL-Server-2025-Semantik präzisiert.
- Die später ergänzten Brainstorm-Themen werden als vier weitere Kandidaten abgegrenzt: kontrollierte PowerShell-Host-Automation (`TC-2026-025`), kontrollierte Python-Ausführung (`TC-2026-026`), externe REST-/Web-Requests (`TC-2026-027`) sowie KI-/Chat-Provider (`TC-2026-028`).
- Allgemeine Raw-Script-, Raw-URL- oder Raw-Prompt-Schnittstellen werden nicht als sichere Capability betrachtet. Provider, Work Types, Datenfreigabe, Credentials und Ergebnisverträge müssen explizit sein.
...
## Nachrecherche: Host-Automation, REST und KI

### PowerShell am Host

SQL Server Agent kann dokumentiert PowerShell- und CmdExec-Jobsteps ausführen.
Für SQL Server 2019 und neuer empfiehlt Microsoft im PowerShell-Jobstep das
`SqlServer`-Modul. Jobsteps besitzen eigene Proxy-/Service-Identitäten,
Exitcodes, Retry- und Outputoptionen; sie liefern jedoch keinen allgemeinen
synchronen, typisierten Resultset-RPC an die aufrufende Session.

`xp_cmdshell` ist keine gleichwertige Abkürzung. Microsoft empfiehlt für neue
Entwicklung ausdrücklich, es nicht zu verwenden und grundsätzlich deaktiviert
zu lassen. Ein Toolbelt-Provider darf deshalb nur vorab registrierte Work Types
mit typisierten Parametern ausführen. Beliebiger PowerShell- oder CmdExec-Text
bleibt außerhalb des Vertrags.

Folgerung: `TC-2026-025` trennt einen kontrollierten externen Worker als
Hauptoption von einem optionalen SQL-Agent-Provider für grobe Windows-Aufgaben.
Identität, Output, Timeout, Cancellation, Audit und Secret-Redaction sind
Pflichtbestandteile.

### Python in SQL Server und auf dem Host

Machine Learning Services führt Python und R mit
`sp_execute_external_script` in einem Extensibility-Framework aus. Das ist
geeignet für datenorientierte In-database-Verarbeitung, aber kein allgemeiner
Host-Automationsvertrag. Ab SQL Server 2022 werden gewünschte Python-/R-Runtimes
und Packages nicht mehr automatisch mit SQL Setup installiert; Runtime- und
Paketversionen werden dadurch noch deutlicher Teil der Deployment- und
Reproduzierbarkeitsgrenze.

Folgerung: `TC-2026-026` behandelt mindestens zwei Provider getrennt:
allowlisted datenorientierte Capabilities über Machine Learning Services und
einen externen Worker für Datei-, Netzwerk- oder Host-Aufgaben. Rohes Python
aus Benutzereingaben und dynamische Paketinstallation sind kein sicherer
Default.

### REST und Web Requests

SQL Server 2025 besitzt mit `sys.sp_invoke_external_rest_endpoint` eine native,
standardmäßig deaktivierte HTTPS-Funktion. Sie akzeptiert URL, Payload, Header,
Methode und weitere Optionen. Microsoft weist ausdrücklich auf das Risiko
unerlaubter Zugriffe und Datenübertragung hin. SQL Server 2019 und 2022 besitzen
diese native Capability nicht.

Ein SQL-CLR-Backport mit `EXTERNAL_ACCESS` wäre keine portable Hauptlösung:
`EXTERNAL_ACCESS` und `UNSAFE` werden unter SQL Server Linux nicht unterstützt.
Ein externer Worker ist daher der plausiblere Compatibility-Provider.

Folgerung: `TC-2026-027` benötigt Endpoint-Allowlist, HTTPS, sichere
Credentials, SSRF-/Redirect-Schutz, Größen- und Timeoutgrenzen,
Content-Type-Prüfung, Retry-/Idempotenzregeln, Audit und Redaction. Lange
Netzwerkaufrufe unter offenen Datenbanklocks sind zu vermeiden.

### KI und Chat

SQL Server 2025 kann External Models registrieren und mit
`AI_GENERATE_EMBEDDINGS` Vektoren erzeugen. `CREATE EXTERNAL MODEL` dokumentiert
unter anderem Azure-OpenAI-, OpenAI- und lokale Ollama-Beispiele. Diese
Embedding-Abstraktion ist jedoch kein allgemeiner Vertrag für Chat, Prompting,
Tool Calling oder vertrauenswürdige strukturierte Antworten.

Folgerung: `TC-2026-028` trennt Embeddings, deterministische
Extraktion/Klassifikation und generative Antworten. Generative Provider bauen
gegebenenfalls auf `TC-2026-027` auf und benötigen zusätzlich
Datenklassifikation, explizite Freigabe externer Übertragung,
Prompt-Injection-Grenzen, Modell-/API-Versionierung, Kosten- und Tokenlimits,
Content Policy sowie Outputvalidierung. KI-Ausgaben dürfen nie ungeprüft als
SQL, DDL oder privilegierter Work Type ausgeführt werden.
...
#### Kontrollierte externe Provider

Die neuen Kandidaten `TC-2026-025` bis `TC-2026-028` teilen eine
Providergrenze, sind aber keine einzelne Capability:

- PowerShell beschreibt kontrollierte Host-Automation;
- Python trennt In-database-Datenverarbeitung von Host-Automation;
- REST beschreibt Transport, Endpoint- und Credentialsemantik;
- KI beschreibt Modell-, Daten-, Kosten- und Outputgovernance.

Gemeinsam sind Allowlist statt Raw Code, typisierte Parameter und Ergebnisse,
Least Privilege, Correlation, Timeout, Cancellation, Audit und Redaction.
...
bleibt historisch erhalten und fortschreibbar. Die zuerst vorhandenen Themen
wurden gegen Engine-Semantik und Repository-Grenzen vorgeprüft. Die späteren
Host-, REST- und KI-Themen wurden als `TC-2026-025` bis `TC-2026-028`
formalisiert; ihre Originalzeilen sind regelkonform durchgestrichen und mit
Änderungsvermerken versehen.
...
| PowerShell am Host | SQL Server Agent kann PowerShell-/CmdExec-Jobsteps ausführen; `xp_cmdshell` ist für neue Entwicklung nicht empfohlen | kontrollierter externer Worker oder optionaler Agent-Provider; keine Raw-Script-Schnittstelle | erlaubte Work Types, Identität, Sync/Async, Output-Schema, Timeout, Abbruch, Windows/Linux |
| Python am Host | Machine Learning Services führt Python datenorientiert über `sp_execute_external_script` aus | In-database-Python und allgemeine Host-Automation als getrennte Provider | reale Use Cases, Packages, Runtimeversion, Ressourcen, Input-/Outputtypen, Plattform |
| REST-/Web-Requests | SQL Server 2025 besitzt nativ `sys.sp_invoke_external_rest_endpoint`; 2019/2022 nicht | nativer 2025-Provider plus optionaler externer Compatibility-Provider | Endpoint-Allowlist, Auth, SSRF, Redirects, Payload, Timeout, Retry, Idempotenz |
| KI/Chat | SQL Server 2025 besitzt External Models und native Embedding-Erzeugung | Embeddings und generative Antworten als getrennte Capabilities | Datenfreigabe, Modell, Prompt-Injection, Output-Schema, Kosten, Rate Limits, Audit |
...
- **External Resource Capabilities** wie Datei, Verzeichnis, Archiv-, Host- und Netzwerkzugriff benötigen eine eigene Security- und Providerentscheidung.
...
Keine dieser Einordnungen ist eine Implementierungsfreigabe. Auch ein
`researched`-Kandidat benötigt vor Implementierung die separate Besprechung von
Zweck, Schnittstelle, Alternativen, Risiken und Scope sowie die anschließende
ausdrückliche Benutzerfreigabe.
...
1. Die noch offene Windows-, Collation-, Grenz-, Recovery- und Performancevalidierung von `toolbelt_core.USP_PrepareResultTable` priorisieren.
...
9. `TC-2026-025` bis `TC-2026-028` erst anhand konkreter erlaubter Host-, REST- und KI-Use-Cases priorisieren; Raw Code und beliebige Endpunkte bleiben ausgeschlossen.
...
- [PowerShell-Schritte in SQL Server Agent](https://learn.microsoft.com/en-us/powershell/sql-server/run-windows-powershell-steps-in-sql-server-agent?view=sqlserver-ps)
- [PowerShell-Jobstep erstellen](https://learn.microsoft.com/en-us/ssms/agent/create-a-powershell-script-job-step)
- [xp_cmdshell-Konfigurationshinweis](https://learn.microsoft.com/en-us/sql/database-engine/configure-windows/xp-cmdshell-server-configuration-option?view=sql-server-ver17)
- [SQL Server Machine Learning Services](https://learn.microsoft.com/en-us/sql/machine-learning/sql-server-machine-learning-services?view=sql-server-ver17)
- [sp_execute_external_script](https://learn.microsoft.com/en-us/sql/relational-databases/system-stored-procedures/sp-execute-external-script-transact-sql?view=sql-server-ver17)
- [sys.sp_invoke_external_rest_endpoint](https://learn.microsoft.com/en-us/sql/relational-databases/system-stored-procedures/sp-invoke-external-rest-endpoint-transact-sql?view=sql-server-ver17)
- [CLR-Programmierung und Linux-Grenzen](https://learn.microsoft.com/en-us/sql/relational-databases/clr-integration/common-language-runtime-clr-integration-programming-concepts?view=sql-server-ver17)
